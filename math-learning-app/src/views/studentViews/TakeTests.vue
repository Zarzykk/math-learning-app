  ...
  <BaseDashboard>
    <v-container fluid class="full-height">
      ...
      <div class="section half-height">
        <h2 class="section-title">Przyszłe testy</h2>
        <v-row>
          <v-col
...
export default {
  name: "TakeTests",
  components: {WorkBody, WorkHeader, BaseDashboard},
  data() {
    return {
  ...
  computed: {
    // Filtrujemy dane na podstawie daty
    upcomingItems() {
  ...
  methods: {
    async fetchData() {
      try {
        const response = await apiService.fetchStudentAssignments();
